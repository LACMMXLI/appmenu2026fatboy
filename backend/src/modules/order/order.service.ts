...
export class OrderService {
  ...
  async deleteAllOrders() {
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const statusHistory = await tx.orderStatusHistory.count();
      const printJobs = await tx.orderPrintJob.count();
      const preservedCustomers = await tx.customer.count();
      const deleted = await tx.order.deleteMany();

      return {
        deletedOrders: deleted.count,
        deletedOrderItems: orderItems,
        deletedStatusHistory: statusHistory,
        deletedPrintJobs: printJobs,
        preservedCustomers,
      };
    });

    if (result.deletedOrders > 0) {
      this.ordersGateway.notifyOrdersPurged(result.deletedOrders);
    }
    return result;
  }
  ...
  async listOrders(filters: { branchId?: string; limit?: number; cursor?: string; query?: string; statuses?: OrderStatus[] }) {
    ...
    const rows = await this.prisma.order.findMany({
      ...
      include: {
        items: true,
        deliveryTask: {
          include: { driver: { select: { id: true, name: true, username: true } } },
        },
      },
      orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
      take: limit + 1,
  ...
  async transitionOrder(
    ...
    const order = await this.prisma.$transaction((tx) => this.transitionOrderInTransaction(tx, orderId, to, opts));

    this.notifyCommittedTransition(order);

    return order;
  }

  /** Shared by delivery completion so its task and order close atomically. */
  async transitionOrderInTransaction(
    ...
    const printDocumentType = to === OrderStatus.PREPARING
      ? PrintDocumentType.PRODUCTION
      : to === OrderStatus.READY
        ? PrintDocumentType.CUSTOMER
        : null;
    if (printDocumentType) {
      await tx.orderPrintJob.upsert({
        where: { orderId_documentType: { orderId, documentType: printDocumentType } },
        update: {},
        create: { id: randomUUID(), orderId, branchId: current.branchId, documentType: printDocumentType },
      });
    }
  ...
  notifyCommittedTransition(order: { id: string; branchId: string; customerId: string | null; status: string }) {

  ...
  async transitionOrder(
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      if (printDocumentType) {
        // The composite unique key makes each operational transition create
        // at most one document. The upsert also keeps deployments compatible
        // with PRODUCTION jobs generated by the previous ACCEPTED workflow.

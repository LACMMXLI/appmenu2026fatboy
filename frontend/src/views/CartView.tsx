...
export function CartView({ onNavigate }: CartViewProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto pt-[42px] pb-[42px] px-3">
      ...
      <div className="border-t border-outline pt-6 mb-6 flex flex-col gap-2 animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
        <div className="flex justify-between items-end">
          <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">SUBTOTAL</span>
          <span className="font-display text-2xl tracking-wide text-white">${subtotal}</span>
        </div>
        ...
        {redeemPointsChecked && discount > 0 && (
          <div className="flex justify-between items-end text-primary">
            ...
            <span className="font-display text-2xl tracking-wide">-${discount}</span>
          </div>
        )}

        <div className="flex justify-between items-end border-t border-outline/30 pt-3">
          <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">TOTAL A PAGAR</span>
          <span className="font-display text-4xl tracking-wide text-accent">${total}</span>
        </div>
      </div>

      {/* Points Redemption Panel */}
      {isAuthenticated && customer && customer.points > 0 && (
        <div className="w-full mb-8 bg-surface border border-outline/50 p-4 rounded-xl animate-fade-in-up" style={{ animationDelay: '0.55s' }}>
      ...
      <div className="w-full mb-8 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">NOTAS DEL PEDIDO</h3>
        <textarea 
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder="Escribe aquí alguna nota general..." 
          className="w-full bg-surface border border-outline rounded-xl p-4 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary resize-none h-24 transition-all"
        ></textarea>
      </div>

      {/* Branch Selection */}
      <div className="w-full mb-8 animate-fade-in-up" style={{ animationDelay: '0.7s' }}>
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">SUCURSAL DE PREFERENCIA</h3>
        <div className="grid grid-cols-2 gap-3">
          {branches.map((branch) => (
            ...
            >
              <span className="font-bold text-center text-[15px] leading-tight block">Fatboy<br/>{branch.name}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {error && <p className="text-sm text-primary mb-4 text-center">{error}</p>}

      <div className="w-full mb-4 flex flex-col items-center animate-fade-in-up" style={{ animationDelay: '0.9s' }}>
        <Button 
          size="lg" 
        ...
        <p className="text-xs text-gray-400 mt-4 text-center">Se registrará en el sistema y se notificará por WhatsApp.</p>
      </div>
    </div>
  );
}
